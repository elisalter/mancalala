...
def updateBoardState(boardState, player, colummChoice):
    ...
    boardSide = player - 1;
    numberOfStones = boardState[player - 1][colummChoice];
    ...
    for i in range(1, numberOfStones+1):
        if (boardState[boardSide][currentColumn] == None):

        boardState[boardSide][columnToMove-i] = boardState[player-1][columnToMove-i] + 1;
